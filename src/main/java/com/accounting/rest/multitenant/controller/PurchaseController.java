...
public class PurchaseController {
	...
	@Autowired
	public PurchaseController(FolioNumberServices folioNumberServices, AccountsTypeServices accountsTypeServices,
	...
	public ResponseEntity<String> addBooks(@RequestBody PurchaseModel books) {
		...
		for (BookDetails bookDetails : books.getBookDetails()) {
			// The Line Of Code Used For The Sum Of The icgst Amount For All Purchase Goods
			icgst += bookDetails.getBookDetailsCgstAmount();

			// The Line Of Code Used For The Sum Of The isgst Amount For All Purchase Goods
		...
		// Here "Purchase Account" Is The Default Debited Transactional Accounts
		Debit_Account_1.setBooksAccount_Ref(accountsServices.getAccountIdByAccountName("Purchase Account"));
		...
		if (icgst != 0) {
			...
			// Here "INPUT CGST" Will Be The 2nd Debited Transaction Account If Its Applied
			Debit_Account_2.setBooksAccount_Ref(accountsServices.getAccountIdByAccountName("INPUT CGST"));

			// Here "Current Assets" Is The Category Of "Input cgst" Account
			...
			Debit_Account_2.setBooksDebitAmount(icgst);
		...
		if (isgst != 0) {
			...
			// Here "INPUT SGST" Will Be The 2nd Debited Transaction Account If Its Applied
			Debit_Account_3.setBooksAccount_Ref(accountsServices.getAccountIdByAccountName("INPUT SGST"));

			// Here "Current Assets" Is The Category Of "INPUT SGST" Account
...
//			booksOfDebit.setBookInfo_Ref(bookInfo.getBookInfo_Ref());
			...
			Debit_Account_3.setBooksDebitAmount(isgst);
		...
		if (igst != 0) {
			...
			// Here "INPUT IGST" Will Be The 2nd Debited Transaction Account If Its Applied
			Debit_Account_4.setBooksAccount_Ref(accountsServices.getAccountIdByAccountName("INPUT IGST"));

			// Here "Current Assets" Is The Category Of "INPUT IGST" Account
		...
		List<Books> debitBooks = new ArrayList<>();
...
//InventoryItems Ready
		...
		if (books.getTransectionalAccounts().getTransactionAccountName().equals("Sundry Creditors")) {
			bookInfo.setBookInfoType_Ref(bookTypeServices.getBookTypeId("Purchase Book").getBookTypeId());
		} else {
			// The Line Of Code Used For _Refentifying The Book Type
		...
		bookInfo.setBookInfoNarration("Purchase");
		bookInfo.setInvoiceNo(books.getInvoiceNo());
		bookInfo.setInvoiceDate(books.getInvoiceDate());
